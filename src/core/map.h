...
//       const render::map_tile_t* get_tile_ptr_lua(const uint32_t &index) const;
      render::light_map_tile_t get_tile(const uint32_t &index) const;
...
//       void resize_structures_buffer(const size_t &size);
    ...
    constexpr size_t map_hex_count = map::hex_count_d(map::detail_level);
//     constexpr size_t accel_hex_count = map::hex_count_d(map::accel_struct_detail_level);
  }
}
