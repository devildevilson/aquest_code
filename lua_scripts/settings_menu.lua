...
function options_menu_window(ctx, menu, data)
  local abs = math.abs;
  ...
  local function check_focus(ctx, current_focus, current_next, bounds)
  ...
  if current_window == "main_menu" then
    menu:escape()
...
function graphics_options_window(ctx, menu, settings)
  local abs = math.abs;
  local dx, dy = ctx:mouse_delta()
  local mouse_movement = not (abs(dx) <= constants.epsilon and abs(dy) <= constants.epsilon)
  local fbw, fbh = input.get_framebuffer_size()
  local video_count = settings.graphics:video_modes_count()
  ...
  if nk.window_begin(ctx, "graphics_options", {0, 0, fbw, fbh}, window_flags) then
    ...
    if nk.combo_begin(ctx, nil, "Video modes", {300, 300}) then
      for i=0,video_count-1 do
        local width, height, refresh_rate = settings.graphics:get_video_mode(i)
        local text = ""
        if settings.graphics.fullscreen then text = tostring(width) .. " " .. tostring(height) .. " " .. tostring(refresh_rate)
        ...
        if selected then
          ...
          settings.graphics.video_mode = i
        end
      end
      nk.combo_end(ctx)
    end
    ...
    if settings.graphics.fullscreen then
      local width, height, refresh_rate = settings.graphics:get_video_mode(settings.graphics.video_mode)
      local text = tostring(width) .. " " .. tostring(height) .. " " .. tostring(refresh_rate)
      nk.label(ctx, "Current video mode: " .. text, nk.TEXT_ALIGN_LEFT)
    else
      local text = tostring(settings.graphics.width) .. " " .. tostring(settings.graphics.height)
      nk.label(ctx, "Current resolution: " .. text, nk.TEXT_ALIGN_LEFT)
    end

    local abc123
    nk.layout_space_push(ctx, menu_layout.entries[3])
    settings.graphics.fullscreen, abc123 = nk.option(ctx, "Fullscreen", settings.graphics.fullscreen) -- , changed
  ...
  if current_graphics_focus ~= 0 and check_event_click(choose) then
    ...
    if current_graphics_focus == 3 then settings.fullscreen = not settings.fullscreen end
    if current_graphics_focus == 4 then settings.projection = not settings.projection end
    if current_graphics_focus == 5 then menu:escape() end
    print("choose " .. current_window)
  end
end
