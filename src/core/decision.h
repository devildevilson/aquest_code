...
//       script::target_t check_input(script::context* ctx) const;
    ...
    struct compiled_decision {
      // нужно сохранить дополнительно персонажа для которого вызывается решение
      const core::character* character;
      const decision* d;
      script::context ctx;
      bool used;
      
      compiled_decision(const core::character* character, const decision* d, const script::context &ctx);
      compiled_decision(const compiled_decision &copy) = delete;
      compiled_decision(compiled_decision &&move) = default;
      compiled_decision & operator=(const compiled_decision &copy) = delete;
      compiled_decision & operator=(compiled_decision &&move) = default;
