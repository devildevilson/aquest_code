  ...
  namespace utils {
    namespace internal {
      void setup_lua_decision(sol::state_view lua) {
        ...
        // думаю что run нужно вытащить в отдельные функции, там должны быть так же проверки, кто запустил, можно ли сейчас и проч
        // все функции так или иначе влияющие на текущее состояние игрового процесса, нужно вынести отдельно чтобы следить за ними было проще
