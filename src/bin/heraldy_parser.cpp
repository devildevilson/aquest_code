...
namespace devils_engine {
  namespace utils {
    ...
    static render::heraldy_layer_t empty_layer() {
      return {
        { GPU_UINT_MAX },
        { GPU_UINT_MAX },
        0,
    ...
    render::image_t find_image(render::image_controller* controller, const std::string_view &img_data) {
      std::string_view img_id;
      uint32_t layer;
      bool mirror_u;
      bool mirror_v;
      const bool ret = render::parse_image_id(img_data, img_id, layer, mirror_u, mirror_v);
      if (!ret) throw std::runtime_error("Bad image id " + std::string(img_data));
      ...
      const auto img = view->get_image(layer, mirror_u, mirror_v);
      return img;
    }
    
    render::heraldy_layer_t parse_heraldy_layer(render::image_controller* controller, const sol::table &table) {
      auto l = empty_layer();
      
      if (const auto proxy = table["stencil"]; proxy.valid()) {
        const auto stencil_image_data = proxy.get<std::string_view>();
        l.stencil = find_image(controller, stencil_image_data);
      }
      
      if (const auto proxy = table["image"]; proxy.valid()) {
        const auto image_data = proxy.get<std::string_view>();
        l.image = find_image(controller, image_data);
      }
...
//         }
      ...
      if (auto proxy = table["colors"]; proxy.valid()) {
        ...
        size_t counter = 0;
        for (const auto &pair : color_table) {
          if (counter >= 4) break;
          if (!pair.second.is<size_t>()) continue;
          const size_t data = pair.second.as<size_t>();
      ...
      if (auto proxy = table["coords"]; proxy.valid()) {
        ...
        size_t counter = 0;
        for (const auto &pair : vec4_table) {
          if (counter >= 4) break;
          if (!pair.second.is<float>()) continue;
      ...
      if (auto proxy = table["reverse_stencil_alpha"]; proxy.valid()) {
        const bool data = proxy;
        l.stencil_type = l.stencil_type | (REVERSE_STENCIL_ALPHA_BIT * uint32_t(data));
      }
...
//       const size_t aligned = align_to(heraldy_buffer_size, 16);
      ...
      for (size_t i = 0; i < heraldy_tables.size(); ++i) {
        ...
        const render::heraldy_layer_t l = parse_heraldy_layer(controller, table);
