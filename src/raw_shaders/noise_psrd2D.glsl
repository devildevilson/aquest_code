...
vec3 psrd_mod289(vec3 x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}

// Permutation polynomial (ring size 289 = 17*17)
vec3 psrd_permute(vec3 x) {
  return psrd_mod289(((x*34.0)+10.0)*x);
}

// Hashed 2-D gradients with an extra rotation.
// (The constant 0.0243902439 is 1/41)
vec2 psrd_rgrad2(vec2 p, float rot) {
#if 0
// Map from a line to a diamond such that a shift maps to a rotation.
  float u = psrd_permute(psrd_permute(p.x) + p.y) * 0.0243902439 + rot; // Rotate by shift
  u = 4.0 * fract(u) - 2.0;
...
//
vec3 psrd_psrdnoise(vec2 pos, vec2 per, float rot) {
  ...
  vec2 g0 = psrd_rgrad2(vec2(iuw.x, ivw.x), rot);
  vec2 g1 = psrd_rgrad2(vec2(iuw.y, ivw.y), rot);
  vec2 g2 = psrd_rgrad2(vec2(iuw.z, ivw.z), rot);
...
//
vec3 psrd_psdnoise(vec2 pos, vec2 per) {
  return psrd_psrdnoise(pos, per, 0.0);
}
...
//
float psrd_psrnoise(vec2 pos, vec2 per, float rot) {
...
// at the minimal cost of three extra additions.
//
float psrd_psnoise(vec2 pos, vec2 per) {
  return psrd_psrnoise(pos, per, 0.0);
}
...
// and the second and third components are the x and y partial derivatives.
//
vec3 psrd_srdnoise(vec2 pos, float rot) {
  // Offset y slightly to hide some rare artifacts
  pos.y += 0.001;
  // Skew to hexagonal grid
  vec2 uv = vec2(pos.x + pos.y*0.5, pos.y);
...
// at the minimal cost of three extra additions.
//
vec3 psrd_sdnoise(vec2 pos) {
  return psrd_srdnoise(pos, 0.0);
}
...
// without the analytical derivative.
//
float psrd_srnoise(vec2 pos, float rot) {
  // Offset y slightly to hide some rare artifacts
  pos.y += 0.001;
  // Skew to hexagonal grid
  vec2 uv = vec2(pos.x + pos.y*0.5, pos.y);
...
//
float psrd_snoise(vec2 pos) {
  return psrd_srnoise(pos, 0.0);
}
