...
layout(location = 1) in flat image_t in_image;
...
void main() {
  ...
  const uint index = valid_image ? get_image_index(in_image) : 0;
  const uint layer = valid_image ? get_image_layer(in_image) : 0;
