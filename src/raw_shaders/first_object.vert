...
void main() {
  ...
  const uint tile_index  = in_tile_index & 0xffffff;
  const uint point_index = gl_VertexIndex;
  // фундаментальная трудность заключается в том что значение gl_InstanceIndex будет сильно варьироваться,
  // нужно где то записывать индекс конкретного инстанса, можно записывать в первом байте
  const uint instance_index = (in_tile_index >> 24) & 0xff;
