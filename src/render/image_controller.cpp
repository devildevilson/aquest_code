...
namespace devils_engine {
  namespace render {
    //static const re2::RE2 image_regex("(\\w+)([.]\\d+)?([.]uv|vu|v|u)?");
    
    bool parse_image_id(const std::string_view &str, std::string_view &image_id, uint32_t &layer_index, bool &mirror_u, bool &mirror_v) {
      // str - строка вида "image_id.123.uv"
      image_id = "";
      layer_index = 0;
      mirror_u = false;
      mirror_v = false;
      
      // чет не понимаю как тут правильно сделать
//       std::array<std::string_view, 3> views;
//       //const size_t count = divide_token(str, ".", 3, views.data());
//       const size_t count = rdivide_token(str, ".", 3, views.data());
//       
//       size_t current_index = 0;
//       const bool is_u  = views[current_index] == "u";
//       const bool is_v  = views[current_index] == "v";
//       const bool is_uv = views[current_index] == "uv";
//       const bool is_vu = views[current_index] == "vu";
//       const bool valid_mirror = is_u || is_v || is_uv || is_vu;
//       current_index += size_t(valid_mirror);
//       size_t num = 0;
//       const auto res = std::from_chars(views[current_index].begin(), views[current_index].end(), num);
//       const bool valid_number = res.ec == std::errc();
//       current_index += size_t(valid_number);
//       image_id = views[current_index];
//       if (!(count == SIZE_MAX && valid_mirror && valid_number)) return false;
